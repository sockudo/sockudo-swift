...
enum ProtocolCodec {
    static func encodeEnvelope(_ envelope: [String: Any], format: SockudoWireFormat) throws -> EncodedPayload {
        switch format {
        case .json:
    ...
    static func decodeEnvelope(_ message: URLSessionWebSocketTask.Message, format: SockudoWireFormat) throws
        -> DecodedEnvelope
    {
        switch format {
        case .json:
    ...
    static func decodeEvent(_ message: URLSessionWebSocketTask.Message, format: SockudoWireFormat) throws -> SockudoEvent {
        ...
        return SockudoEvent(
            ...
            sequence: (envelope["__delta_seq"] as? NSNumber)?.intValue ?? (envelope["sequence"] as? NSNumber)?.intValue,
            conflationKey: envelope["__conflation_key"] as? String ?? envelope["conflation_key"] as? String,
...
private enum MessagePackCodec {
    ...
    private static func decodeEnvelopeValue(_ value: Any) -> Any? {
        ...
        if let map = value as? [String: Any] {
            return Dictionary(uniqueKeysWithValues: map.map { key, entryValue in
        ...
        if let extras = envelope["extras"] as? [String: Any] {
            ...
            if let headers = extras["headers"] as? [String: Any] {
                for key in headers.keys.sorted() {
                    ...
                    } else if let number = encodedValue as? NSNumber, CFGetTypeID(number) != CFBooleanGetTypeID() {
